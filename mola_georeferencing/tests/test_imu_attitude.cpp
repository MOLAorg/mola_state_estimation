...
void run_single_test(double roll_deg, double pitch_deg, double sensor_yaw_deg, bool use_yaml)
{
    ...
    for (int i = 0; i < 5; i++)
    {
        ...
        if (!use_yaml)
        {
            ...
            const mrpt::math::TVector3D a_sensor =
                T_enu_to_sensor.getRotationMatrix().transpose() * g_enu;
        ...
        else
        {
            ...
            const mrpt::poses::CPose3D  T_enu_to_veh = T_enu_to_map + T_map_to_veh;
            const mrpt::math::TVector3D a_veh =
                T_enu_to_veh.getRotationMatrix().transpose() * g_enu;
    ...
    const mrpt::math::TVector3D z_est  = T_est.getRotationMatrix().col(2);
    const mrpt::math::TVector3D z_true = T_enu_to_map.getRotationMatrix().col(2);
...
int main()
{
    try
    {
        ...
        const std::vector<std::pair<double, double>> angle_cases = {
            {30.0, 0.0}, {0.0, 20.0}, {0.0, 90.0}};

        const std::vector<double> sensor_yaws = {0.0, 90.0};
