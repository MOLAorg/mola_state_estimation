...
| odom motion-model noise | `odom_motion_model_a1..a4` / `MOLA_ODOM_MOTION_MODEL_A1..A4`, `_min_std_xy` / `_min_std_phi_deg` | MRPT `CActionRobotMovement2D` mmGaussian-model defaults, `min_std` floors tighter (1e-3 m / 0.1 deg) | Proportional noise over each `fuse_odometry()` increment: `a1` [m/m], `a2` [m/deg], `a3` [deg/m], `a4` [deg/deg] (grows with distance/angle, not a fixed per-step sigma). Lower `a1..a4` for accurate encoders with no slip; raise for loose terrain. |
