...
| odom motion-model noise | `odom_motion_model_a1..a4` / `MOLA_ODOM_MOTION_MODEL_A1..A4`, `_min_std_xy` / `_min_std_phi_deg` | MRPT `CActionRobotMovement2D` mmGaussian-model defaults. `min_std_xy`/`min_std_phi_deg` are NOT the effective floor: `fuse_odometry_locked()` adds a further `+1e-4` to every diagonal covariance entry afterwards, i.e. sigma >= 0.01 m / ~0.57 deg regardless of these two | Proportional noise over each `fuse_odometry()` increment: `a1` [m/m], `a2` [m/rad], `a3` [rad/m], `a4` [rad/rad] (grows with distance/angle, not a fixed per-step sigma) -- units verified against MRPT's implementation, not its docs comment, which says `a2`/`a3` are in degrees and is wrong (the code multiplies them by `CPose2D::phi()`, radians). Lower `a1..a4` for accurate encoders with no slip; raise for loose terrain. |
